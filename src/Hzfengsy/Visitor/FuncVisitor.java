...
public class FuncVisitor extends MBaseVisitor<IRBaseNode>
{
    ...
    public FuncVisitor(Functions _functions, Classes _classes, ErrorReporter _reporter) {
        functions = _functions; classes = _classes; reporter = _reporter;
    }
    ...
    private FuncType func_print() {
        Vector<BaseType> parameter = new Vector<>(); parameter.add(classes.get("string"));
    ...
    private FuncType func_toString() {
        Vector<BaseType> parameter = new Vector<>(); parameter.add(classes.get("int"));
        return new FuncType(classes.get("string"), parameter, "string toString(int i)", "toString");
    }

    private void loadInsideFunction() {
        functions.insert("getInt", func_getInt()); functions.insert("print", func_print());
        functions.insert("println", func_println()); functions.insert("getString", func_getString());
        functions.insert("toString", func_toString());
    }

    @Override
    public IRBaseNode visitProg(MParser.ProgContext ctx) {
        if (classStack.empty()) loadInsideFunction(); visitChildren(ctx);
        if (classStack.empty()) if (!checkMainFunc()) error("main function error"); return null;
    }

    @Override
    public IRBaseNode visitClas(MParser.ClasContext ctx) {
        BaseType clas = null;
        try { clas = classes.getClass(ctx.id().getText()); } catch (Exception e) { error(e.getMessage()); }
        IRBaseNode ans = new IRTypeNode(clas, true); classStack.push(ans); if (ctx.prog() != null) visit(ctx.prog());
        classStack.pop(); return ans;
    }
    ...
    public IRBaseNode visitFunc(MParser.FuncContext ctx) {
        String funcName = ctx.id().getText(); String className = null;
        String funcLine = ctx.class_stat().getText() + ' ' + funcName + '(' + ctx.stat_list().getText() + ')';
        if (ctx.class_stat() == null) {
            if (classStack.empty()) error("error construction function.");
            else {
                UserType userClass = (UserType) classStack.peek().getType(); String userClassName = userClass.getName();
                if (userClassName.equals(funcName)) className = "void";
                else error("error construction function.");
            }
        }
        else className = ctx.class_stat().getText(); Vector<BaseType> list = visit(ctx.stat_list()).getTypeList(); try {
    ...
    public IRBaseNode visitStatListCombine(MParser.StatListCombineContext ctx) {
        ...
        list.addAll(visit(ctx.stat_list(1)).getTypeList()); return new IRTypeListNode(list);
    }

    @Override
    public IRBaseNode visitStatList(MParser.StatListContext ctx) {
        if (ctx.getText().equals("")) return new IRTypeListNode(new Vector<>());
        String className = ctx.class_stat().getText(); BaseType type = null;
        try { type = classes.getClass(className); } catch (Exception e) { error(e.getMessage()); }
        Vector<BaseType> list = new Vector<>(); list.add(type); return new IRTypeListNode(list);
    }

    @Override
    public IRBaseNode visitId_Define(MParser.Id_DefineContext ctx) {
        String varName = ctx.id().getText(); String className = ctx.class_stat().getText();
        if (className.equals("void")) error("cannot define void variable"); if (!classStack.empty()) try {
            UserType userClass = (UserType) classStack.peek().getType();
            userClass.insertMemberVar(varName, classes.getClass(className));
        } catch (Exception e) { error(e.getMessage()); } return new IRBaseNode();
    }
